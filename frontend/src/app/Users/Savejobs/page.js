  ...
  const fetchJobs = async (page, search = searchTerm) => {
    ...
    try {
      const response = await axios.get(
        `http://127.0.0.1:3001/get_save_job?page=${page}&search=${search}`,
  ...
  const handleSaveJob = async (jobId) => {
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        // Check if the page is now empty after unsaving
        if (jobs.length === 1 && currentPage > 1) {
          // Move to the previous page and fetch its jobs
          setCurrentPage((prevPage) => prevPage - 1);
          await fetchJobs(currentPage - 1); // Explicitly fetch the previous page
        }
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[50vh] bg-[#F9FAFB] rounded-lg p-8 shadow-md">
            ...
            <h2 className="text-3xl font-bold text-[#0073b1] mb-4">No Saved Jobs</h2>
            <p className="text-lg text-gray-600 text-center max-w-md">
              You haven't saved any jobs yet. Browse through the available jobs and save your favorite ones to apply later.
            </p>
          </div>
        ) : (
          <>
              ...
              {jobs.map((job) => (
                ...
                >
                  ...
                  <div className="mt-6 flex justify-between items-center">
                    ...
                    {/* Updated Save Job Button with enhanced UI */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleSaveJob(job.job_id);
                      }}
                      className="px-4 py-2 rounded-lg text-sm font-medium bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
                    >
